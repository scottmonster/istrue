  ...
  if is_valid_expression "$input"; then
    case "$EXP_TYPE" in
      math)
        ...
        # Use eval to execute in a way that variable assignments affect the caller
        # Note: This still won't persist through function boundaries, but it's the best we can do
        if eval "(( $eval_expr ))" 2>/dev/null; then
          return 0
        else
          return 1
        fi
...
# Convenience function - clearest possible implementation
isfalse() {
  if istrue "$@"; then
    # istrue returned 0 (true), so return 1 (false)
    return 1
  else
    # # istrue returned non-zero (false or error), so return 0 (true)  
    # # Only return 0 if it was actually false (1), propagate errors (2+)
    # local result=$?
    # if [[ $result -eq 1 ]]; then
    #   return 0
    # else
